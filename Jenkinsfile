pipeline {
    ...
    steps {
      script {
        ...
        if (commitMessage.contains("[skip ci]")) {
          echo "Skipping pipeline due to skip [skip ci] tag in commit message."
    ...
    steps {
      script {
        ...
        checkout({$class: 'GitSCM',
          ...
          userRemoteConfigs:
            [url: repoUrl, credentialsIdL 'github-credential']
        })
  ...
  stage('Build and Push Docker Image') {
    steps {
      script {
        def runNumber  = currentBuild.runNumber
    ...
    steps {
      script {
        sh """
          ...
          git remote set-url origin https://${GITHUB_CREDENTIALS_USR}:${GITHUB_CREDENTIALS_PSW}@git@github.com:Mini-Soccer-Project/user-service.git
  ...
  stage('Deploy to Remote Server') {
    steps {
      script {
        def targetDir  = "/home/fribsa/mini-soccer-project/user-service"
